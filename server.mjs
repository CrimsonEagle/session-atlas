...
const server=http.createServer(async(req,res)=>{
 ...
 try {
  ...
  if(req.method==='GET') {
   ...
   if(url.pathname==='/api/bootstrap')return json(200,{token,settings,startup:await startupStatus(),rates:{...rates,...remotePrices()},pricing:priceSyncStatus(),dataDir,bridgeScript:path.join(root,'bridge','atlas-statusline.mjs')});
