...
Portable, local usage analytics for **Claude Code, OpenAI Codex, and Hermes Agent** on Windows and Linux. Install and run Atlas on the same computer as the tools whose sessions you want to analyze. No installer, npm installation, API keys, or external fonts. Analytics work offline; only the optional pricing update requires internet access. Requires **Node.js 22.13 or newer** and a current browser.
...
- Recursive session trees for Claude Code and Codex parent sessions with explicitly associated subagents or review agents, plus Hermes session links. Hierarchy is visible in the session list, session details, and expandable task view; self usage stays separate from usage including linked sessions, while uncertain or missing relationships remain visible instead of being estimated.
...
- Session details with model responses or Hermes usage measurements, branch, working directory, input/output/cache/reasoning tokens, and a context timeline where available. Model or context-window changes, data gaps, and explicitly logged compactions are marked; missing window sizes do not produce invented percentages.
- Five-hour and weekly limits for Codex and Claude Code, including plan, measurement time, and reset: Codex from the latest logged `rate_limits`; Claude Code from `cachedUsageUtilization` in `.claude.json` and optionally from the status-line bridge. The newest measurement and its source are shown. Expired measurements appear as unknown, not 0%.
...
- Dedicated cost-limit view in the limit details: for every historical measurement, known API costs since the start of the reported five-hour or weekly window are divided by usage and extrapolated to 100%. The view shows individual measurements by default or one arithmetic mean per reset window as a dot. The selected grouping and date range are saved in this browser. Both views use the same continuous, robust trend calculated from the individual measurements, with low usage and incomplete prices receiving less weight. The latest value, median, range, and pricing coverage remain visible. Faint envelope curves show the trend's range implied by rounding the reported usage (for whole percentages, ±0.5 percentage points); they do not account for unknown model prices or provider accounting differences. Usage of 0%, missing resets, and entirely unknown prices do not produce invented values.
- Rolling costs for the last five hours or seven days for Codex and Claude Code. These are **not identical billing windows** and are not a substitute for percentage-based subscription limits.
...
Available data sources (the local folders are read by default):
...
| Hermes Agent | Local `~/.hermes/state.db` and `~/.hermes/profiles/*/state.db`; on Windows Hermes uses `%LOCALAPPDATA%/hermes` by default |

`~` means the current user's home directory on either system. `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, and `HERMES_HOME` override the respective default root directories. Additional absolute source folders can be added in the settings, including accessible UNC or WSL directories on Windows. Browser data is not collected.

### Local Hermes Agent Sessions

Install and start Atlas on the Hermes computer. It automatically looks for Hermes in its platform default directory, or in `HERMES_HOME` when set. You can add other Hermes home directories under **Settings → Data Sources**. Each home includes its default `state.db` and databases of named profiles.

Atlas reads only session and per-model usage metadata from the local SQLite databases. It does not read prompt or response content. Per-model usage keeps model switches and auxiliary tasks separate; aggregate session counters fill any remaining older usage. Parent links are displayed as linked sessions; Hermes sessions explicitly marked as `subagent` appear as subagents. The first import places cumulative usage at its last recorded activity time, and later scans add counter changes. These entries are usage measurements rather than individual model responses. Atlas prefers Hermes's recorded cost when its status is known, then falls back to locally stored prices. Read errors appear after refresh, while previously imported data remains visible. Hermes subscription limits are not currently available through this source.
...
App data is stored in a versioned set under `.local/states/<id>/`; `.local/active-state.json` points atomically to the active set. It includes `usage-cache.json`, `settings.json`, `model-prices.json`, and `price-history.json`. Existing flat files are copied into an initial set on first launch. The server binds to `127.0.0.1` by default and can use a specific IPv4 address via `ATLAS_HOST`; foreign origins and changes without a local session token are rejected.
...
Under **Settings → Update Model Prices → Update Prices**, the app downloads the public catalogs from [LiteLLM](https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json), [models.dev](https://models.dev/api.json), and [OpenRouter](https://openrouter.ai/api/v1/models). OpenRouter model prices are used for Hermes routes when Hermes has no known recorded cost. Models with a per-request surcharge are skipped because Atlas currently supports token prices only. ccusage does not need to be installed. Only pricing files are downloaded; no session data is transmitted.

Prices are stored in the active local data set and loaded offline on the next launch. Manual prices take precedence over online prices; the bundled table remains available as a fallback for otherwise unknown models. Newer source fetches take precedence, with LiteLLM preferred over models.dev when both are fetched at the same time. If a source fails, successful catalogs are still used and a message is displayed; if all three fail, existing prices are preserved. Fetch times and model counts appear next to the button. A fetch time is not a confirmed date on which provider prices changed.
...
On Linux, set `ATLAS_PORT`, `ATLAS_HOST`, `ATLAS_DATA_DIR`, `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, and `HERMES_HOME` in the desktop session environment if the autostarted app should use non-default values. A terminal-only export is not available to the next login's desktop session.
