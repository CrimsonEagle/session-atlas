...
export function createDetailViews(options) {
 ...
 function sessionHierarchy(session) {
  ...
  if(!tree||(tree.nodes.length===1&&!session.subagent&&!tree.unassigned))return '';
  const rows=tree.nodes.map(({node,depth})=>{
   const current=node.session.id===session.id;
   const label=node.reason||(node.session.relationType==='guardian_review'?'Prüf-Agent':node.session.subagent?'Subagent':'Hauptsession');
