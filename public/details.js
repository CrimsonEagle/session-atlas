...
export function createDetailViews(options) {
 ...
 function aggregateTable(sessions,summary){
  ...
  const cache=session=>cacheRatio(session),model=session=>[...new Set(session.events.map(event=>event.model))].join(', '),value=(session,key)=>key==='title'?(session.title||basename(session.cwd)):key==='model'?model(session):key==='share'?session.tokens:key==='requests'?session.requests:key==='tokens'?session.tokens:key==='cost'?session.unknown===session.requests&&session.requests?null:session.cost:Date.parse(session.lastActivity||0);
