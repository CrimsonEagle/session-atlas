...
import {attachTooltips,axisLabel,nextSort,seriesChart,sortableHeader,sortRows,tipAttr} from './charts.js';
import {contextHistoryView} from './context-history.js';
import {taskTreeForSession} from './tasks.js';
...
export function createDetailViews(options) {
 const {dialog,content,getData,getFiltered,getScope,applyFilter,loadSessionDetails,esc,num,compact,money,date,basename,toolName,toolTag,costText}=options,activeLocale=()=>typeof options.locale==='function'?options.locale():options.locale||'de-DE',activeTools=()=>typeof options.getActiveTools==='function'?options.getActiveTools():TOOL_IDS;
 const pageSize=12,responsePageSize=20;
 let state={view:null},history=[],searchTimer;

 const percent=value=>(value*100).toLocaleString(activeLocale(),{maximumFractionDigits:1})+' %';
 ...
 function delta(current,previous){if(previous==null)return '';if(previous===0)return current===0?'0 %':'Neu';const value=(current-previous)/previous*100;return `${value>0?'+':''}${value.toLocaleString(activeLocale(),{maximumFractionDigits:0})} % zur Vorperiode`;}
 ...
 function kpis(summary,previous,sessionMode=false){
  const p=previous?analytics(previous):null,items=[
   ...
   ['Modellantworten',num(summary.requests),delta(summary.requests,p?.requests),`${compact(summary.output)} Output-Tokens`],
    ['Cache-Anteil',percent(summary.cacheRatio),p?`${(summary.cacheRatio-p.cacheRatio)*100>=0?'+':''}${((summary.cacheRatio-p.cacheRatio)*100).toLocaleString(activeLocale(),{maximumFractionDigits:1})} Prozentpunkte`:'',`${compact(summary.cache)} gelesene Tokens`]
  ];
 ...
 function aggregateTable(sessions,summary){
  let rows=grouped(sessions,'sessions'),query=(state.search||'').trim().toLowerCase();if(query)rows=rows.filter(session=>[session.name,session.title,session.cwd,session.repository,session.branch,session.sessionId,...session.events.map(event=>event.model)].join(' ').toLowerCase().includes(query));
  if(state.agent&&state.agent!=='all')rows=rows.filter(session=>(state.agent==='subagent')===Boolean(session.subagent));
  const cache=session=>cacheRatio(session),model=session=>[...new Set(session.events.map(event=>event.model))].join(', '),value=(session,key)=>key==='title'?sessionLabel(session):key==='model'?model(session):key==='share'?session.tokens:key==='requests'?session.requests:key==='tokens'?session.tokens:key==='cost'?session.unknown===session.requests&&session.requests?null:session.cost:Date.parse(session.lastActivity||0);
  rows=sortRows(rows,state.sessionSort,state.sessionSortDirection,value);
  ...
  const head=(label,key,numeric=false)=>sortableHeader(label,key,{activeKey:state.sessionSort,direction:state.sessionSortDirection,context:'detail-session',numeric});
  return `<section class="detail-section detail-session-list"><div class="detail-section-head"><div><h3>Zugehörige Sessions</h3><p>${num(rows.length)} Treffer${state.dimension?' in der Unterauswahl':''}</p></div><div class="detail-table-tools"><input id="detail-session-search" type="search" value="${esc(state.search||'')}" placeholder="Sessions durchsuchen" aria-label="Sessions durchsuchen"><select id="detail-agent-filter" aria-label="Sessiontyp"><option value="all">Alle Sessiontypen</option><option value="main" ${state.agent==='main'?'selected':''}>Hauptsessions</option><option value="subagent" ${state.agent==='subagent'?'selected':''}>Subagents</option></select></div></div><div class="table-wrap"><table><thead><tr>${head('Session','title')}${head('Modell / Branch','model')}${head('Anteil','share',true)}${head('Antworten','requests',true)}${head('Tokens','tokens',true)}${head('API-Kosten','cost',true)}${head('Aktivität','activity',true)}</tr></thead><tbody>${shown.map(session=>{const secondary=sessionSecondaryId(session),subtitle=[secondary,basename(session.repository),session.relationType==='related'?'Verknüpfte Session':session.subagent?'Subagent':''].filter(Boolean).join(' · ');return `<tr><td><button class="row-button" data-session="${esc(session.id)}"><span class="row-title" title="${esc(sessionLabel(session))}">${esc(sessionLabel(session))}</span><span class="row-subtitle">${esc(subtitle)}</span></button></td><td>${esc(model(session)||'Nicht protokolliert')}<span class="row-subtitle">${esc(session.branch||'Ohne Branch')}</span></td><td class="numeric">${percent(summary.tokens?session.tokens/summary.tokens:0)}</td><td class="numeric">${num(session.requests)}</td><td class="numeric"><strong>${compact(session.tokens)}</strong><span class="row-subtitle">${percent(cache(session))} Cache</span></td><td class="numeric">${costText(session)}</td><td class="numeric muted">${date(session.lastActivity)}</td></tr>`;}).join('')||'<tr><td colspan="7">Keine Sessions für diese Auswahl.</td></tr>'}</tbody></table></div><div class="table-footer"><span>${rows.length?`${state.sessionPage*pageSize+1}–${Math.min((state.sessionPage+1)*pageSize,rows.length)} von ${rows.length}`:'0 Treffer'}</span><div class="pagination"><button data-detail-page="-1" ${state.sessionPage===0?'disabled':''} aria-label="Vorherige Seite">‹</button><span>Seite ${state.sessionPage+1} / ${pages}</span><button data-detail-page="1" ${state.sessionPage+1>=pages?'disabled':''} aria-label="Nächste Seite">›</button></div></div></section>`;
 }
 ...
 function sessionHierarchy(session) {
  const tree=taskTreeForSession(session,getData().sessions);
  if(!tree||(tree.nodes.length===1&&!session.parentId&&!session.subagent&&!tree.unassigned))return '';
  const rows=tree.nodes.map(({node,depth})=>{
   const current=node.session.id===session.id;
   const label=node.reason||(node.session.relationType==='guardian_review'?'Prüf-Agent':node.session.relationType==='related'?'Verknüpfte Session':node.session.subagent?'Subagent':'Hauptsession');
   const title=sessionLabel(node.session),secondary=sessionSecondaryId(node.session),content=`<span><strong title="${esc(title)}">${esc(title)}</strong><small>${esc([secondary,label].filter(Boolean).join(' · '))}</small></span><span class="detail-agent-metrics"><span><small>Eigen</small><strong>${compact(node.own.tokens)}</strong></span><span><small>Mit Kindern</small><strong>${compact(node.total.tokens)}</strong></span><span><small>Kosten</small><strong>${costText(node.own)}</strong></span></span>`;
   return `<div class="detail-agent-node ${current?'current':''}" role="treeitem" aria-level="${depth+1}" style="--task-depth:${depth}"><span class="task-connector" aria-hidden="true"></span>${current?`<div class="detail-agent-link" aria-current="true">${content}</div>`:`<button class="detail-agent-link" data-session="${esc(node.session.id)}">${content}</button>`}</div>`;
  }).join('');
  return `<section class="detail-section detail-agent-tree"><div class="detail-section-head"><div><h3>Session-Hierarchie</h3><p>${num(tree.nodes.length)} Sessions · rekursiv über direkte Parent-Beziehungen</p></div></div><div class="detail-agent-nodes" role="tree" aria-label="Session-Hierarchie">${rows}</div></section>`;
 }
 function renderSession(){
  const raw=getData().sessions.find(session=>session.id===state.id);if(!raw)return;const filtered=getFiltered().find(session=>session.id===state.id),session=state.scope==='filtered'&&filtered?{...raw,events:filtered.events}:raw,summary=analytics([session]),range=eventRange(session.events,session),models=[...new Set(session.events.map(event=>event.model))],tiers=[...new Set(session.events.map(event=>event.tier).filter(Boolean))],geos=[...new Set(session.events.map(event=>event.geo).filter(Boolean))],pricingModes=[...new Set(session.events.map(event=>event.pricingMode).filter(Boolean))],pricingRefs=[...new Set(session.events.map(event=>event.priceSnapshotId).filter(Boolean))],pricingRules=[...new Set(session.events.map(event=>event.pricingRuleVersion).filter(Boolean))];
  const responseValue=(event,key)=>key==='activity'?Date.parse(event.time||0):key==='model'?event.model:key==='input'?event.input:key==='cache'?(event.cache||0)+(event.write||0):key==='output'?event.output:key==='reasoning'?event.reasoning:key==='tokens'?tokenCount(event):event.cost;
  let responses=sortRows(session.events,state.responseSort,state.responseSortDirection,responseValue);const pages=Math.max(1,Math.ceil(responses.length/responsePageSize));state.responsePage=Math.min(state.responsePage||0,pages-1);const shown=responses.slice(state.responsePage*responsePageSize,(state.responsePage+1)*responsePageSize),contextRatio=raw.contextWindow?raw.contextUsed/raw.contextWindow:0;
  const timelineItems=state.scope==='filtered'&&session.events.length?(raw.contextTimeline||[]).filter(item=>Date.parse(item.time)>=Date.parse(range.firstActivity)&&Date.parse(item.time)<=Date.parse(range.lastActivity)):(raw.contextTimeline||[]),contextMarkup=`<div class="detail-pricing-evidence"><strong>Preisbewertung:</strong> ${esc(pricingModes.map(mode=>mode==='reported'?'von Hermes gemeldet':mode==='historical'?'historisch hinterlegt':'aktuell').join(', ')||'aktuell')} · Regel ${esc(pricingRules.join(', ')||'nicht protokolliert')} · ${pricingRefs.length?`${num(pricingRefs.length)} Preisstand-Referenz${pricingRefs.length===1?'':'en'}`:'kein zugeordneter Stand'}</div>`+contextHistoryView({timeline:timelineItems,lastUsed:raw.contextUsed,lastWindow:raw.contextWindow,compact,date,esc,locale:activeLocale()});
  const responseHead=(label,key,numeric=false)=>sortableHeader(label,key,{activeKey:state.responseSort,direction:state.responseSortDirection,context:'detail-response',numeric});
  setContent(`${header(sessionLabel(raw),`${state.scope==='filtered'?periodLabel():'Gesamte Session'} · ${raw.relationType==='related'?'Verknüpfte Session':raw.subagent?'Subagent':'Hauptsession'}`,toolTag(raw.tool))}<div class="detail-body detail-analysis" data-tip-host><div class="detail-scope-row"><div class="segments"><button data-detail-scope="filtered" class="${state.scope==='filtered'?'active':''}" ${!filtered?'disabled':''}>Im gewählten Zeitraum</button><button data-detail-scope="all" class="${state.scope==='all'?'active':''}">Gesamte Session</button></div></div><dl class="detail-meta detail-meta-wide"><div><dt>Session-ID</dt><dd>${esc(raw.sessionId)}</dd></div><div><dt>Repository</dt><dd>${esc(raw.repository)}</dd></div><div><dt>Branch</dt><dd>${esc(raw.branch||'–')}</dd></div><div><dt>Aktivität im Ausschnitt</dt><dd>${date(range.firstActivity)} / ${date(range.lastActivity)}</dd></div><div><dt>Modelle / Service-Tiers</dt><dd>${esc(models.join(', ')||'Nicht protokolliert')} · ${esc(tiers.join(', ')||'Standard')}</dd></div><div><dt>Reasoning / Region</dt><dd>${esc(raw.effort||'Nicht protokolliert')}${geos.length?` · ${esc(geos.join(', '))}`:''}</dd></div><div class="context-meta"><dt>Kontext laut letztem Log</dt><dd>${raw.contextWindow?`${compact(raw.contextUsed)} / ${compact(raw.contextWindow)} Tokens · ${percent(contextRatio)}`:'Nicht verfügbar'}</dd>${raw.contextWindow?`<div class="track" ${tipAttr({title:'Kontextfenster',rows:[['Belegt',compact(raw.contextUsed)],['Frei',compact(Math.max(0,raw.contextWindow-raw.contextUsed))],['Fenster',compact(raw.contextWindow)]],total:['Ausgelastet',percent(contextRatio)],note:'Stand der letzten protokollierten Antwort'})}><i style="width:${Math.min(100,contextRatio*100)}%"></i></div>`:''}</div></dl>${sessionHierarchy(raw)}${kpis(summary,null,true)}${timeline([session],state.metric)}${contextMarkup}<div class="detail-breakdown-grid session-breakdowns">${breakdownPanel([session],'model',false)}${breakdownPanel([session],'tier',false)}</div>${tokenBreakdown(summary)}${quality(summary,raw.malformed)}<section class="detail-section"><div class="detail-section-head"><div><h3>${raw.tool==='hermes'?'Nutzungsmesspunkte':'Modellantworten'}</h3></div></div><div class="table-wrap"><table><thead><tr>${responseHead('Zeitpunkt','activity')}${responseHead('Modell','model')}${responseHead('Input','input',true)}${responseHead('Cache','cache',true)}${responseHead('Output','output',true)}${responseHead('Reasoning','reasoning',true)}${responseHead('Gesamt','tokens',true)}${responseHead('API-Kosten','cost',true)}</tr></thead><tbody>${shown.map(event=>`<tr><td>${date(event.time)}</td><td>${esc(event.model)}<span class="row-subtitle">${esc(event.tier||'standard')}${event.effort?` · ${esc(event.effort)}`:''}</span></td><td class="numeric">${num(event.input)}</td><td class="numeric">${num((event.cache||0)+(event.write||0))}</td><td class="numeric">${num(event.output)}</td><td class="numeric">${num(event.reasoning)}</td><td class="numeric"><strong>${num(tokenCount(event))}</strong></td><td class="numeric">${event.cost===null?'Unbekannt':money(event.cost)}</td></tr>`).join('')||'<tr><td colspan="8">Diese Session enthält keine Nutzungsmetriken im gewählten Zeitraum.</td></tr>'}</tbody></table></div><div class="table-footer"><span>${responses.length?`${state.responsePage*responsePageSize+1}–${Math.min((state.responsePage+1)*responsePageSize,responses.length)} von ${responses.length}` :raw.tool==='hermes'?'0 Messpunkte':'0 Antworten'}</span><div class="pagination"><button data-response-page="-1" ${state.responsePage===0?'disabled':''}>‹</button><span>Seite ${state.responsePage+1} / ${pages}</span><button data-response-page="1" ${state.responsePage+1>=pages?'disabled':''}>›</button></div></div></section></div>`);
 }
 ...
 function handleClick(event){
  ...
  const sortButton=target.closest('[data-sort-context^="detail-"]');if(sortButton){const next=sortButton.dataset.sortContext==='detail-session'?nextSort(state.sessionSort,state.sessionSortDirection,sortButton.dataset.sortKey,sortButton.dataset.sortDefault):nextSort(state.responseSort,state.responseSortDirection,sortButton.dataset.sortKey,sortButton.dataset.sortDefault);if(sortButton.dataset.sortContext==='detail-session'){state.sessionSort=next.key;state.sessionSortDirection=next.direction;state.sessionPage=0;}else{state.responseSort=next.key;state.responseSortDirection=next.direction;state.responsePage=0;}render();return;}
  if(target.closest('[data-apply-group]')){applyFilter(state.kind,state.key);close();}
 }
 function handleChange(event){if(event.target.id==='detail-agent-filter'){state.agent=event.target.value;state.sessionPage=0;render();}}
