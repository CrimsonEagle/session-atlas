...
export function taskView({sessions,allSessions=sessions,sort='activity',esc=String,num=String,compact=String,costText=row=>String(row.cost),basename=value=>value,toolTag=value=>value,date=String}) {
 ...
 const flatten=(node,depth=0)=>[{node,depth},...node.children.flatMap(child=>flatten(child,depth+1))];
 const card=(root,unassigned=false)=>{
  const nodes=flatten(root),contributing=nodes.filter(({node})=>!node.contextOnly),models=new Map();
  ...
  return `<details class="task-card"><summary><span class="task-caret" aria-hidden="true">›</span><span class="task-title"><strong>${esc(root.session.title||basename(root.session.cwd))}</strong><small>${esc(basename(root.session.repository))}${unassigned?` · ${esc(root.reason)}`:root.contextOnly?' · Parent außerhalb der Auswahl':''}</small></span><span>${toolTag(root.session.tool)}</span><span class="numeric"><strong>${num(root.contributing)}</strong><small>${root.contributing===1?'Session':'Sessions'}</small></span><span class="numeric"><strong>${compact(root.total.tokens)}</strong><small>Tokens</small></span><span class="numeric"><strong>${costText(root.total)}</strong><small>API-Kosten</small></span><span class="numeric task-activity">${date(root.lastActivity)}</span></summary><div class="task-body"><div class="task-tree" role="tree" aria-label="Aufgabenbaum">${nodes.map(({node,depth})=>`<div class="task-node ${node.contextOnly?'context-only':''}" role="treeitem" aria-level="${depth+1}" style="--task-depth:${depth}"><span class="task-connector" aria-hidden="true"></span><button class="row-button" data-session="${esc(node.session.id)}"><span class="row-title">${esc(node.session.title||basename(node.session.cwd))}</span><span class="row-subtitle">${node.contextOnly?'Orientierung · außerhalb der Auswahl':node.session.relationType==='guardian_review'?'Prüf-Agent':node.session.subagent?'Subagent':'Hauptsession'}</span></button><span class="numeric"><small>Eigen</small><strong>${node.contextOnly?'–':compact(node.own.tokens)}</strong></span><span class="numeric"><small>Mit Kindern</small><strong>${compact(node.total.tokens)}</strong></span><span class="numeric"><small>Kosten</small><strong>${node.contextOnly?'–':costText(node.own)}</strong></span></div>`).join('')}</div><div class="task-distributions"><section><h3>Verteilung auf Agents</h3>${agentRows.map(({node})=>`<button data-session="${esc(node.session.id)}"><span>${esc(node.session.title||node.session.sessionId.slice(0,12))}</span><strong>${compact(node.own.tokens)}</strong></button>`).join('')||'<p>Keine Nutzungsdaten im Zeitraum.</p>'}</section><section><h3>Verteilung auf Modelle</h3>${modelRows.map(([model,tokens])=>`<div><span>${esc(model)}</span><strong>${compact(tokens)}</strong></div>`).join('')||'<p>Keine Modelle im Zeitraum.</p>'}</section></div></div></details>`;
 };
