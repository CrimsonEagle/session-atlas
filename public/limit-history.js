...
function costChart(points,esc,date,money,range,aggregation) {
 const usable=points.filter(point=>Number.isFinite(point.estimatedLimit));
 if(!usable.length)return '<div class="comparison-empty limit-cost-empty">Für diesen Zeitraum lässt sich noch kein Kostenlimit hochrechnen. Benötigt werden ein positiver Usage-Wert, ein gemeldeter Reset und bepreiste Nutzungsereignisse im selben Fenster.</div>';
 const width=900,height=274,left=82,right=18,top=20,bottom=38,times=usable.map(point=>Date.parse(point.sourceObservedAt)),dataMin=Math.min(...times),dataMax=Math.max(...times),min=range.start??dataMin,max=range.end??dataMax,span=Math.max(1,max-min),maximum=Math.max(...usable.map(point=>point.estimatedLimit),.01),x=value=>left+(Date.parse(value)-min)/span*(width-left-right),y=value=>top+(maximum-value)/maximum*(height-top-bottom),grid=[0,.25,.5,.75,1].map(factor=>{const value=maximum*(1-factor),at=top+(height-top-bottom)*factor;return `<g class="limit-cost-grid"><line x1="${left}" y1="${at}" x2="${width-right}" y2="${at}"/><text x="${left-10}" y="${at+4}" text-anchor="end">${esc(money(value))}</text></g>`;}).join('');
...
function usageView({points,allPoints,chartPoints,toolbar,sortKey,sortDirection,esc,date,range}) {
 ...
 return `<div class="detail-body limit-history-detail">${toolbar}<div class="limit-history-legend"><span><i class="window-300"></i>5 Stunden</span><span><i class="window-10080"></i>Wöchentlich</span><small>Flächen enden an Resetgrenzen.</small></div>${usageChart(chartPoints,esc,date,range)}<div class="table-wrap"><table><thead><tr>${head('Messzeitpunkt','observed')}${head('Fenster','window')}${head('Auslastung','usage',true)}${head('Quelle','source')}${head('Reset','reset')}</tr></thead><tbody>${rows.map(point=>`<tr><td>${esc(date(point.sourceObservedAt))}</td><td>${label(point.windowMinutes)}</td><td class="numeric"><strong>${point.usedPercent.toLocaleString('de-DE')} %</strong></td><td>${source(point.source)}</td><td>${point.resetsAt?esc(date(point.resetsAt)):'Nicht gemeldet'}</td></tr>`).join('')}</tbody></table></div><p class="detail-footnote">${points.length} von ${allPoints.length} Messpunkten im gewählten Zeitraum · maximal 100 werden in der Tabelle angezeigt${chartPoints.length<points.length?` · Diagramm auf ${chartPoints.length} Punkte verdichtet`:''}.</p></div>`;
}

function costView({points,allPoints,toolbar,sortKey,sortDirection,esc,date,money,range,aggregation}) {
 const estimatable=points.filter(point=>Number.isFinite(point.estimatedLimit)),averaged=aggregation==='window',displayPoints=averaged?averageCostLimitWindows(points):estimatable,value=(point,key)=>key==='observed'?Date.parse(point.sourceObservedAt):key==='window'?point.windowMinutes:key==='measurements'?point.measurements:key==='usage'?point.usedPercent:key==='spent'?point.windowCost:key==='estimate'?point.estimatedLimit:key==='coverage'?point.coverage:null,rows=sortRows(displayPoints,sortKey,sortDirection,value).slice(0,100),head=(text,key,numeric=false)=>sortableHeader(text,key,{activeKey:sortKey,direction:sortDirection,context:'limit-history',numeric}),unknown=displayPoints.filter(point=>averaged?point.incompleteMeasurements:point.unknownCosts).length;
 ...
 const table=averaged?`<table><thead><tr>${head('Letzter Messpunkt','observed')}${head('Fenster','window')}${head('Messpunkte','measurements',true)}${head('Ø Usage','usage',true)}${head('Fenstermittel','estimate',true)}${head('Ø Preisabdeckung','coverage',true)}</tr></thead><tbody>${rows.map(point=>`<tr><td>${esc(date(point.sourceObservedAt))}</td><td>${label(point.windowMinutes)}<span class="row-subtitle">Reset ${esc(date(point.resetsAt))}</span></td><td class="numeric">${point.measurements.toLocaleString('de-DE')}</td><td class="numeric">${point.usedPercent.toLocaleString('de-DE',{maximumFractionDigits:1})} %</td><td class="numeric"><strong>${money(point.estimatedLimit)}</strong><span class="row-subtitle">${money(point.minEstimatedLimit)} – ${money(point.maxEstimatedLimit)}</span></td><td class="numeric${point.incompleteMeasurements?' coverage-partial':''}">${point.coverage===null?'–':`${(point.coverage*100).toLocaleString('de-DE',{maximumFractionDigits:0})} %`}</td></tr>`).join('')}</tbody></table>`:`<table><thead><tr>${head('Messzeitpunkt','observed')}${head('Fenster','window')}${head('Kosten bis dahin','spent',true)}${head('Usage','usage',true)}${head('100%-Schätzung','estimate',true)}${head('Preisabdeckung','coverage',true)}</tr></thead><tbody>${rows.map(point=>`<tr><td>${esc(date(point.sourceObservedAt))}</td><td>${label(point.windowMinutes)}</td><td class="numeric">${money(point.windowCost)}</td><td class="numeric">${point.usedPercent.toLocaleString('de-DE')} %</td><td class="numeric"><strong>${money(point.estimatedLimit)}</strong></td><td class="numeric${point.unknownCosts?' coverage-partial':''}">${point.coverage===null?'–':`${(point.coverage*100).toLocaleString('de-DE',{maximumFractionDigits:0})} %`}</td></tr>`).join('')}</tbody></table>`;
 return `<div class="detail-body limit-history-detail limit-cost-detail">${toolbar}<p class="limit-cost-intro">Geschätzter API-Gegenwert des nutzbaren Kontingents: ${intro}</p>${costSummary(displayPoints,money,aggregation)}<div class="limit-history-legend"><span><i class="window-300"></i>5 Stunden</span><span><i class="window-10080"></i>Wöchentlich</span><small>${summary}</small></div>${costChart(displayPoints,esc,date,money,range,aggregation)}${unknown?`<div class="notice compact-notice">${notice} Diese Werte sind unvollständig und können höher ausfallen.</div>`:''}<div class="table-wrap">${table}</div><p class="detail-footnote">${summary} im gewählten Zeitraum · ${allPoints.length} Messpunkte insgesamt. Die Hochrechnung ist eine lokale Näherung aus API-Preisen, keine Auskunft des Anbieters über ein Geldlimit.</p></div>`;
}

export function limitHistoryView({history,sessions=[],tool,mode='usage',aggregation='window',sortKey='observed',sortDirection='desc',period='30',from='',to='',now=Date.now(),esc=String,date=String,money=value=>`$${Number(value).toFixed(2)}`}) {
 ...
 return mode==='usage'?usageView({points,allPoints,chartPoints,toolbar,sortKey,sortDirection,esc,date,range}):costView({points,allPoints,toolbar,sortKey,sortDirection,esc,date,money,range,aggregation});
}
