...
const localDate=value=>new Date(value).toLocaleDateString('de-DE',{day:'2-digit',month:'short',year:'numeric'});

function changeText(current,previous,{points=false}={}) {
 if(points){const delta=(current-previous)*100;return `${delta>0?'+':''}${delta.toLocaleString('de-DE',{maximumFractionDigits:1})} Prozentpunkte`;}
 if(previous===0)return current===0?'Keine Änderung':'Neu';
 const delta=(current-previous)/previous*100;return `${delta>0?'+':''}${delta.toLocaleString('de-DE',{maximumFractionDigits:0})} %`;
}

function lineChart(currentSessions,previousSessions,currentRange,previousRange,metric,format) {
 ...
 const points=(series,kind,label)=>series.map((point,index)=>{
  const payload={title:`${label} · ${localDate(`${point.key}T00:00:00`)}`,rows:[[metric==='cost'?'API-Schätzung':metric==='requests'?'Modellantworten':'Tokens',format(point.value),kind]],note:`Tag ${index+1} im Zeitraum`};
...
export function comparisonView({currentSessions,previousSessions,currentRange,previousRange,metric='tokens',dimension='repository',esc,num,compact,money,basename,toolName}) {
 ...
 const kpis=[
  ['Tokens',compact(current.tokens),compact(previous.tokens),changeText(current.tokens,previous.tokens)],
  ['API-Schätzung',current.unknown===current.requests&&current.requests?'–':money(current.cost),previous.unknown===previous.requests&&previous.requests?'–':money(previous.cost),changeText(current.cost,previous.cost)],
  ['Modellantworten',num(current.requests),num(previous.requests),changeText(current.requests,previous.requests)],
  ['Sessions',num(current.sessionCount),num(previous.sessionCount),changeText(current.sessionCount,previous.sessionCount)],
  ['Cache-Anteil',(current.cacheRatio*100).toLocaleString('de-DE',{maximumFractionDigits:1})+' %',(previous.cacheRatio*100).toLocaleString('de-DE',{maximumFractionDigits:1})+' %',changeText(current.cacheRatio,previous.cacheRatio,{points:true})]
 ];
 const rows=comparisonRows(currentSessions,previousSessions,dimension,metric),dimensionLabel={repository:'Projekte',model:'Modelle',tool:'KI-Tools'}[dimension];
 const name=row=>dimension==='repository'?basename(row.name):dimension==='tool'?toolName(row.name):row.name;
 const delta=row=>row.previous===0?(row.current===0?'–':'Neu'):changeText(row.current,row.previous);
 ...
 return `<section class="panel comparison-panel"><div class="panel-head comparison-head"><div><h2>Zeiträume vergleichen</h2><p><span class="comparison-key current"></span>${esc(localDate(currentRange.start))} – ${esc(localDate(currentRange.end))}<span class="comparison-key previous"></span>${esc(localDate(previousRange.start))} – ${esc(localDate(previousRange.end))}</p></div><div class="segments" aria-label="Vergleichsmetrik">${[['tokens','Tokens'],['cost','Kosten'],['requests','Antworten']].map(([key,label])=>`<button data-comparison-metric="${key}" class="${metric===key?'active':''}">${label}</button>`).join('')}</div></div><div class="comparison-kpis">${kpis.map(([label,currentValue,previousValue,deltaValue])=>`<div><span>${esc(label)}</span><strong>${esc(currentValue)}</strong><small>Vergleich ${esc(previousValue)}</small><em>${esc(deltaValue)}</em></div>`).join('')}</div>${lineChart(currentSessions,previousSessions,currentRange,previousRange,metric,format)}${costWarning}<div class="comparison-table-head"><div><h3>Was hat sich verändert?</h3><p>Größte Beiträge zur Veränderung</p></div><div class="segments" aria-label="Vergleichsdimension">${[['repository','Projekte'],['model','Modelle'],['tool','KI-Tools']].map(([key,label])=>`<button data-comparison-dimension="${key}" class="${dimension===key?'active':''}">${label}</button>`).join('')}</div></div><div class="table-wrap"><table><thead><tr><th>${dimensionLabel}</th><th class="numeric">Vergleich</th><th class="numeric">Aktuell</th><th class="numeric">Absolut</th><th class="numeric">Relativ</th></tr></thead><tbody>${rows.map(row=>`<tr><td><button class="row-button comparison-row" data-comparison-filter="${dimension}" data-comparison-value="${esc(row.name)}"><span class="row-title">${esc(name(row))}</span></button></td><td class="numeric">${format(row.previous)}</td><td class="numeric">${format(row.current)}</td><td class="numeric comparison-delta ${row.delta>0?'positive':row.delta<0?'negative':''}">${row.delta>0?'+':''}${format(row.delta)}</td><td class="numeric">${esc(delta(row))}</td></tr>`).join('')||'<tr><td colspan="5">Keine Aktivitäten in beiden Zeiträumen.</td></tr>'}</tbody></table></div></section>`;
}
