...
export function costFor(e, overrides={}) {
 const model=e.model.replace(/-\d{8}$/, '');
 const own=(object,key)=>Object.hasOwn(object,key)?object[key]:undefined;
 const remote=remotePrices();
 const r=own(overrides,e.model)||own(overrides,model)||own(remote,e.model)||own(remote,model)||own(rates,e.model)||own(rates,model);
 if(!r) return null;
 ...
 if((model.startsWith('gpt-6-')||model.startsWith('gpt-5.6-')||model==='gpt-5.5') && e.input+e.cache+e.write>272000) {input*=2;cache*=2;write*=2;output*=1.5;}
 let factor=1;
