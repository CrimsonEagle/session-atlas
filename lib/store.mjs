...
export class Store {
 constructor(cacheFile) {this.cacheFile=cacheFile;this.files={};this.limitHistory=[];this.limitNotified={};this.limitAlerts=[];this.cacheVersion=null;this.cacheNeedsBackup=false;this.repos=new Map();this.repoPending=new Map();this.claudeLimits=null;this.scanCount=0;this.lastScan=null;}
 async load() {try {const x=JSON.parse(await fs.readFile(this.cacheFile,'utf8'));if([2,3,4,5].includes(x.version)){this.files=x.files;this.limitHistory=Array.isArray(x.limitHistory)?x.limitHistory:[];this.limitNotified=x.limitNotified&&typeof x.limitNotified==='object'?x.limitNotified:{};this.cacheVersion=x.version;this.cacheNeedsBackup=x.version<5;}}catch{}}
 ...
 async performScan(settings) {
  ...
  const results=await mapLimit(files,4,async({tool,file})=>{
    ...
    try {
     ...
     const s=restart?newState(tool,file):{...cached,events:{...cached.events},records:{...cached.records},limits:{...cached.limits},limitObservations:{...cached.limitObservations}};
  ...
  if(changed||history.changed) {try {
   ...
   await fs.writeFile(this.cacheFile+'.tmp',JSON.stringify({version:5,files:this.files,limitHistory:this.limitHistory,limitNotified:this.limitNotified}));await fs.rename(this.cacheFile+'.tmp',this.cacheFile);
   this.cacheVersion=5;this.cacheNeedsBackup=false;
