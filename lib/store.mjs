...
import {newState,ingest} from './parser.mjs';
...
function unchanged(cached,st,tool) {
 return cached?.prefixHash&&cached.tool===tool&&cached.size===st.size&&cached.mtime===st.mtimeMs
...
export class Store {
 constructor(cacheFile) {this.cacheFile=cacheFile;this.files={};this.repos=new Map();this.repoPending=new Map();this.claudeLimits=null;this.scanCount=0;this.lastScan=null;}
 async load() {try {const x=JSON.parse(await fs.readFile(this.cacheFile,'utf8'));if([2,3].includes(x.version))this.files=x.files;}catch{}}
 ...
 async performScan(settings) {
  ...
  const results=await mapLimit(files,4,async({tool,file})=>{
    ...
    try {
     ...
     let prefix=createHash('sha256'),restart=!cached||cached.tool!==tool||!cached.prefixHash||st.size<cached.offset||(cached.size===st.size&&cached.mtime!==st.mtimeMs);
     if(!restart) {
     ...
     const s=restart?newState(tool,file):{...cached,events:{...cached.events},records:{...cached.records},limits:{...cached.limits}};
  ...
  if(changed) {try {await fs.mkdir(path.dirname(this.cacheFile),{recursive:true});await fs.writeFile(this.cacheFile+'.tmp',JSON.stringify({version:3,files:this.files}));await fs.rename(this.cacheFile+'.tmp',this.cacheFile);}catch(e){warnings.push(`Cache konnte nicht gespeichert werden: ${e.code}`);}}
 ...
 snapshot(settings) {
  ...
  for(const [id,states] of groupedStates) {
   ...
   for(const s of states)for(const event of Object.values(records?(s.records||{}):(s.events||{}))) {if(seen.has(event.id))continue;seen.add(event.id);session.events.push({...event,cost:costFor(event,settings.prices)});}
  ...
  return {sessions:sessions.sort((a,b)=>(b.lastActivity||'').localeCompare(a.lastActivity||'')),limits,claudeBridge,lastScan:this.lastScan,stats:this.stats,priceDate};
 }
}
