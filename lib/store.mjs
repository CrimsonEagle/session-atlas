...
export class Store {
 constructor(cacheFile,priceHistory=null) {this.cacheFile=cacheFile;this.priceHistory=priceHistory;this.files={};this.limitHistory=[];this.limitHistoryIds=new Set();this.limitHistoryNextPrune=0;this.limitRetentionDays=null;this.limitNotified={};this.limitAlerts=[];this.cacheVersion=null;this.cacheNeedsBackup=false;this.cacheDirty=false;this.repos=new Map();this.repoPending=new Map();this.claudeLimits=null;this.dataRevision=0;this.scanCount=0;this.lastScan=null;}
 async load() {try {const x=JSON.parse(await fs.readFile(this.cacheFile,'utf8'));if([2,3,4,5,6].includes(x.version)){this.files=x.files;this.limitHistory=Array.isArray(x.limitHistory)?x.limitHistory:[];this.limitHistoryIds=new Set(this.limitHistory.map(point=>point.id));this.limitNotified=x.limitNotified&&typeof x.limitNotified==='object'?x.limitNotified:{};this.cacheVersion=x.version;this.cacheNeedsBackup=x.version<6;}}catch{}}
 ...
 async performScan(settings,options={}) {
  ...
  const codexNames=await readCodexSessionNames(settings.codexRoots,warnings);
  ...
  this.stats={durationMs:Date.now()-begun,changed,metadataChanges,bytes,files:Object.keys(this.files).length,scanCount:this.scanCount,warnings};
