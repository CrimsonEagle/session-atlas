...
function inspectJson(name,text){const value=JSON.parse(text);if(!value||typeof value!=='object'||Array.isArray(value))throw Error(`${name} hat keinen gültigen Objekttyp.`);if(name==='usage-cache.json'&&![2,3,4,5,6].includes(value.version))throw Error('Die Cache-Version der Sicherung ist nicht kompatibel.');return value;}
...
export function parseBackup(buffer){
 ...
 const cache=JSON.parse(files['usage-cache.json']),settings=JSON.parse(files['settings.json']),sessionIds=new Set(Object.values(cache.files||{}).map(item=>`${item.tool}:${item.id}`)),sourceRoots=[...(settings.claudeRoots||[]).map(value=>({tool:'claude',path:value})),...(settings.codexRoots||[]).map(value=>({tool:'codex',path:value}))];
