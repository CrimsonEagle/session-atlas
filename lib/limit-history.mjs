...
export function updateLimitHistory(sources,history=[],notified={},options={}) {
 const now=options.now??Date.now(),retentionDays=Number.isFinite(options.retentionDays)?options.retentionDays:90,known=options.knownIds||new Set(history.map(point=>point.id)),added=[];
 ...
 if(options.prune!==false) {
  ...
  added.sort((a,b)=>a.sourceObservedAt.localeCompare(b.sourceObservedAt));
  nextHistory=!history.length||history.at(-1).sourceObservedAt<=added[0].sourceObservedAt?[...history,...added]:[...history,...added].sort((a,b)=>a.sourceObservedAt.localeCompare(b.sourceObservedAt));
 }
 ...
 if(options.prune!==false){const notifiedCutoff=now-(retentionDays+14)*86400000;for(const [key,value] of Object.entries(nextNotified))if(Date.parse(value)<notifiedCutoff)delete nextNotified[key];}
 return {history:nextHistory,notified:nextNotified,alerts,added,changed:added.length>0||nextHistory.length!==history.length||Object.keys(nextNotified).length!==Object.keys(notified).length};
}
